...
- `api/controllers/converterController.js` preserves non-video files as uploaded and converts non-`mp4` videos (including `.mov`) to Android-compatible `.mp4` via `ffmpeg-static` before saving:
  - H.264 / `avc1` High@4.1, `yuv420p`, AAC stereo 48kHz, `+faststart`
  - converted video keeps aspect ratio, caps the long side at `1920`, and normalizes dimensions to even values required by H.264
  - converted video bitrate peaks are capped with ffmpeg VBV settings (`maxrate 12M`, `bufsize 24M`) for better Android playback compatibility
...
- Desktop icon assets are generated by `scripts/build-desktop-icon.js` into `electron/assets/icon.png` and `electron/assets/icon.ico`; current source asset is `frontend/public/Logo.svg`.
- `electron/main.js` uses Windows `.ico` desktop icon metadata (`app.setAppUserModelId` + `BrowserWindow` icon from `icon.ico` + `win.setAppDetails(...)`) for taskbar/relaunch consistency on Windows.
- Desktop build unpacks `electron/assets/*.ico` and `electron/assets/*.png` outside `app.asar`; `electron/main.js` resolves Windows runtime icon from `process.resourcesPath/app.asar.unpacked/electron/assets/icon.ico`.
- Windows desktop build also runs `scripts/electron-after-pack.js` as an `electron-builder` `afterPack` hook and applies icon/version resource metadata directly to `desktop-dist/win-unpacked/PinIt.exe` via `rcedit`.
- Current desktop app version in the root `package.json`: `1.0.1`
...
Update (2026-04-13, laptop deploy script)
